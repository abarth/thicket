...
into a local SQLite database, which will need to be synchronized with `tickets.jsonl`.

The tickets will be identified by a two-letter code, which represents the project using the ticket tracker, and a random hex string,
...
We will also need a way to add comments to issues so that we can track what we learn about tickets over time.
